...
def roll_dice(num_rolls, dice=six_sided):
    ...
    the outcomes any of the outcomes is 1. In that case, return 1.
...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    strscore = str(score**2)
    listscore = list(strscore)
    return 3 + int(min(listscore))
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    oppo = str(opponent_score)
    if len(player) == 1:
        player = '0'+player
    if len(oppo) == 1:
        oppo = '0' + oppo
    for i in range(2):
        if int(player[i]) == int(oppo[i]):
            return False
        elif int(player[i]) < int(oppo[i]):
            pass
        else:
            return False
    return True
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def print_out(score0, score1, running_high=running_high, last_score=last_score):
        score = score1
        if who == 0:
            score = score0
        if running_high < score-last_score:
            print(
                f"Player {who} has reached a new maximum point gain. {score-last_score} point(s)!")
...
def make_averaged(original_function, trials_count=1000):
    ...
    def trial(*args):
        ...
        for i in range(trials_count):
            sum_num += original_function(*args)
        return sum_num/trials_count
    return trial
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    score = {}
    for i in range(1, 11):
        score[i] = (make_averaged(roll_dice, trials_count)(i, dice))
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_added = score + piggy_points(opponent_score)
    if more_boar(score_added, opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    This strategy is return random numrolls 0 to 10 
    """
    # BEGIN PROBLEM 12
    import random
